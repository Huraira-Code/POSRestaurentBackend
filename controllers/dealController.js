...
const createDeal = async (req, res, next) => {
    try {
        ...
        const { name, description, items, dealPrice, adminId, validUntil } = req.body;
        ...
        if (existingDeal) {
            return res.status(400).json({ 
                success: false, 
                message: "Deal name must be unique for this admin. A deal with this name already exists for this restaurant." 
            });
        }
...
const updateDeal = async (req, res, next) => {
    try {
        ...
        const { name, description, items, dealPrice, isActive, validUntil } = req.body;
