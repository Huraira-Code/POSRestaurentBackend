...
//     console.log("order to close", ordersToClose);
...
//       }
//       totalDeliveryFees += order.deliveryFee || 0; // ✅ Add this line
...
//         ...item,
//         totalRevenue: parseFloat(item.completeItem.price.toFixed(2)),
...
//           "CASH ON DELIVERY": parseFloat(
//             paymentMethodSummaryPaid["CASH ON DELIVERY"].toFixed(2)
//           ),

//           "CASH ON ARRIVAL": parseFloat(
//             paymentMethodSummaryPaid["CASH ON ARRIVAL"].toFixed(2)
//           ),
//         },
//         UNPAID: {
...
//           "CASH ON DELIVERY": parseFloat(
//             paymentMethodSummaryUnpaid["CASH ON DELIVERY"].toFixed(2)
//           ),

//           "CASH ON ARRIVAL": parseFloat(
//             paymentMethodSummaryUnpaid["CASH ON ARRIVAL"].toFixed(2)
//           ),
//         },
//       },
//     };
...
const printDailySalesReportAndCloseDay = async (req, res, next) => {
  try {
    ...
    ordersToClose.forEach((order) => {
      ...
      const paymentTypeKey = order.paymentMethod;
      const orderTypeKey = order.orderType || "OTHER"; // fallback if missing
      ...
      if (order.paymentStatus === "PAID") {
        ...
        if (
          paymentBreakdownByType[orderTypeKey]?.PAID?.hasOwnProperty(
            paymentTypeKey
          )
        ) {
          paymentBreakdownByType[orderTypeKey].PAID[paymentTypeKey] +=
            order.totalAmount || 0;
        }
      } else if (order.paymentStatus === "UNPAID") {
        if (paymentMethodSummaryUnpaid.hasOwnProperty(paymentTypeKey)) {
        ...
        if (
          paymentBreakdownByType[orderTypeKey]?.UNPAID?.hasOwnProperty(
            paymentTypeKey
          )
        ) {
          paymentBreakdownByType[orderTypeKey].UNPAID[paymentTypeKey] +=
            order.totalAmount || 0;
        }
      }
    });
    ...
    const salesReport = {
      ...
      paymentMethodSummary: {
        PAID: Object.fromEntries(
          Object.entries(paymentMethodSummaryPaid).map(([k, v]) => [
            k,
            parseFloat(v.toFixed(2)),
          ])
        ),
        UNPAID: Object.fromEntries(
          Object.entries(paymentMethodSummaryUnpaid).map(([k, v]) => [
            k,
            parseFloat(v.toFixed(2)),
          ])
        ),
      ...
      // 🔥 New: include per-orderType breakdown
      paymentBreakdownByType: Object.fromEntries(
        Object.entries(paymentBreakdownByType).map(([type, data]) => [
          type,
          {
            PAID: Object.fromEntries(
              Object.entries(data.PAID).map(([k, v]) => [
                k,
                parseFloat(v.toFixed(2)),
              ])
            ),
            UNPAID: Object.fromEntries(
              Object.entries(data.UNPAID).map(([k, v]) => [
                k,
                parseFloat(v.toFixed(2)),
              ])
            ),
    ...
    res.status(200).json({
      success: true,
      message: "Sales report generated and items marked as closed.",
      report: salesReport,
    });
  } catch (error) {
    console.error("Error in printDailySalesReportAndCloseDay:", error);
