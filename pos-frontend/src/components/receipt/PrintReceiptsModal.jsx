          ...
          <div class="company-header">
            ...
            <div class="items-section">
                    ...
                    <table class="receipt-table">
                      ...
                      <tbody>
                       ${
                         ...
                         processedReceiptData.items.length > 0
                           ? (() => {
                               // Helper to create a unique key based on item name + selected (non-addon) options
                               const getItemKey = (item) => {
                                 const selectedOptions = (item.options || [])
                                   .filter((opt) => !opt.option) // normal (non-addon) options
                                   .map(
                                     (opt) => `${opt.name}:${opt.price || 0}`
                                   )
                                   .sort()
                                   .join("|");
                                 return `${item.name}|${selectedOptions}`;
                               };

                               // 1️⃣ Group main items
                               const groupedItems = {};
                               processedReceiptData.items.forEach((item) => {
                                 ...
                                 if (!groupedItems[key]) {
                                   ...
                                   // Combine "true" options (addons) as well
                                   const allOptions = [
                                     ...(groupedItems[key].options || []),
                                     ...(item.options || []),
                                   ];
                                   groupedItems[key].options = allOptions;
                                 }
                               });

                               // 2️⃣ Generate HTML for grouped items
                               return Object.values(groupedItems)
                                 .map((item) => {
                                   ...
                                   // 3️⃣ Group true-options (addons)
                                   const groupedAddons = Object.values(
                                     (item.options || [])
                                       .filter((opt) => opt.option === true)
                                       .reduce((acc, opt) => {
                                         if (!acc[opt.name]) {
                                           acc[opt.name] = { ...opt, qty: 1 };
                                         } else {
                                           acc[opt.name].qty += 1;
                                         }
                                         return acc;
              ...
              <tr>
                ...
                <td class="unit-price-cell">Rs${itemTotalPrice.toFixed(2)/item.quantity.toFixed(2)}</td>
                <td class="price-cell">Rs${itemTotalPrice.toFixed(2)}</td>
              </tr>
              ${
                ...
                <tr>
                  ...
                  <td class="qty-cell">${opt.qty}</td>
                  <td class="unit-price-cell">Rs${opt.price.toFixed(2)}</td>
                  <td class="price-cell">Rs${(opt.price * opt.qty).toFixed(
                    2
                  )}</td>
                </tr>
              `
